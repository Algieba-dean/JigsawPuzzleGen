...
class PuzzleSolver:
    ...
    def _check_edge_compatibility(
        ...
        # 检查是否是边缘位置
        is_edge_position = (
            row == 0 or row == self.puzzle.rows - 1 or col == 0 or col == self.puzzle.cols - 1
        )
    ...
    def _try_rotations(
        ...
        next_row: int,
        next_col: int,
        solutions: List[List[Tuple[int, int, int, int]]],
    ) -> bool:
        """尝试所有可能的旋转"""
        ...
        for rotation in rotations:
            ...
            if self._check_placement(piece, row, col, current_solution):
                ...
                if self._find_solutions_recursive(
                    next_row, next_col, used_pieces, current_solution, solutions
                ):
    ...
    def _find_solutions_recursive(
        ...
        if (
            (row == 0 and col == 0)
            or (row == 0 and col == self.puzzle.cols - 1)
            or (row == self.puzzle.rows - 1 and col == 0)
            or (row == self.puzzle.rows - 1 and col == self.puzzle.cols - 1)
        ):
    ...
    def _collect_solution(
        self, current_solution: List[List[Optional[JigsawPiece]]]
    ) -> List[Tuple[int, int, int, int]]:
        """收集当前解决方案"""
        return [
            (piece.id, r, c, piece.rotation)
    ...
    def find_all_solutions(
        ...
        used_pieces = set()
        current_solution = [
            [None for _ in range(self.puzzle.cols)] for _ in range(self.puzzle.rows)
        ]
