...
def edge_test_puzzle():
    ...
    # 创建一个边缘片（非角落）
    edge_piece = JigsawPiece(
        2,
        {Direction.UP: 0, Direction.RIGHT: 3, Direction.DOWN: 4, Direction.LEFT: -1},
        is_edge=True,
    )
...
def complex_puzzle():
    ...
    pieces.append(
        JigsawPiece(
            3,
            {Direction.UP: 0, Direction.RIGHT: 0, Direction.DOWN: 5, Direction.LEFT: -3},
            is_corner=True,
        )
    )
    ...
    pieces.append(
        JigsawPiece(
            4,
            {Direction.UP: -2, Direction.RIGHT: 6, Direction.DOWN: 7, Direction.LEFT: 0},
            is_edge=True,
        )
    )
    ...
    pieces.append(
        JigsawPiece(
            5, {Direction.UP: -4, Direction.RIGHT: 8, Direction.DOWN: 9, Direction.LEFT: -6}
        )
    )
    ...
    pieces.append(
        JigsawPiece(
            6,
            {Direction.UP: -5, Direction.RIGHT: 0, Direction.DOWN: 10, Direction.LEFT: -8},
            is_edge=True,
        )
    )
    ...
    pieces.append(
        JigsawPiece(
            7,
            {Direction.UP: -7, Direction.RIGHT: 11, Direction.DOWN: 0, Direction.LEFT: 0},
            is_corner=True,
        )
    )
    ...
    pieces.append(
        JigsawPiece(
            8,
            {Direction.UP: -9, Direction.RIGHT: 12, Direction.DOWN: 0, Direction.LEFT: -11},
            is_edge=True,
        )
    )
    ...
    pieces.append(
        JigsawPiece(
            9,
            {Direction.UP: -10, Direction.RIGHT: 0, Direction.DOWN: 0, Direction.LEFT: -12},
            is_corner=True,
        )
    )
...
def multi_solution_puzzle():
    ...
    pieces.append(
        JigsawPiece(
            1,
            {Direction.UP: 0, Direction.RIGHT: 1, Direction.DOWN: 1, Direction.LEFT: 0},
            is_corner=True,
        )
    )
    ...
    pieces.append(
        JigsawPiece(
            2,
            {Direction.UP: 0, Direction.RIGHT: 0, Direction.DOWN: 1, Direction.LEFT: -1},
            is_corner=True,
        )
    )
    ...
    pieces.append(
        JigsawPiece(
            3,
            {Direction.UP: -1, Direction.RIGHT: 2, Direction.DOWN: 0, Direction.LEFT: 0},
            is_corner=True,
        )
    )
    ...
    pieces.append(
        JigsawPiece(
            4,
            {Direction.UP: -1, Direction.RIGHT: 0, Direction.DOWN: 0, Direction.LEFT: -2},
            is_corner=True,
        )
    )
...
def test_partial_solution(complex_puzzle):
    ...
    non_matching_piece = next(
        p for p in complex_puzzle.pieces if p.is_corner and p.get_edge(Direction.UP) != -1
    )  # 确保上边缘不匹配
...
def test_complex_puzzle_validity(complex_puzzle):
    ...
    for piece in pieces:
        print(
            f"片{piece.id} ({'角落片' if piece.is_corner else '边缘片' if piece.is_edge else '内部片'}):"
        )
        print(f"  上: {piece.get_edge(Direction.UP)}")
        print(f"  右: {piece.get_edge(Direction.RIGHT)}")
        print(f"  下: {piece.get_edge(Direction.DOWN)}")
    ...
    for value, occurrences in all_edges.items():
        ...
        if matching_value in all_edges:
            assert (
                len(all_edges[matching_value]) == 1
            ), f"边缘值 {value} 应该只有一个匹配值 {matching_value}"
            assert len(occurrences) == 1, f"边缘值 {value} 在拼图中出现了多次"
        else:
...
def test_solution_uniqueness(multi_solution_puzzle):
    ...
    # 将解转换为可哈希的形式以检查唯一性
    solution_tuples = [
        tuple(sorted((piece_id, row, col, rot) for piece_id, row, col, rot in solution))
        for solution in solutions
    ]
...
def test_solution_symmetry(multi_solution_puzzle):
    ...
    # 验证某些片可以出现在多个位置
    pieces_with_multiple_positions = [
        piece_id for piece_id, positions in piece_positions.items() if len(positions) > 1
    ]
...
def test_solution_rotation_variants(multi_solution_puzzle):
    ...
    # 验证至少有一个片有多个有效的旋转角度
    pieces_with_multiple_rotations = [
        piece_id for piece_id, rotations in piece_rotations.items() if len(rotations) > 1
    ]
