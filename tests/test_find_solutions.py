...
def create_puzzle(rows: int, cols: int) -> JigsawPuzzle:
    ...
    # 创建一个二维数组来存储边的值
    horizontal_edges = [[i + j * cols for j in range(cols-1)] for i in range(1, rows * cols, cols)]
...
def test_edge_piece_placement(puzzle_with_size: Tuple[JigsawPuzzle, int, int]):
    ...
    for piece_id, row, col, _ in solution:
        ...
        if piece.is_corner:
            ...
            assert row in (0, rows-1) or col in (0, cols-1), f"{rows}x{cols} 拼图边缘片放置位置错误"
        else:
            assert row not in (0, rows-1) and col not in (0, cols-1), f"{rows}x{cols} 拼图普通片放置位置错误"
